...
namespace BLL.Services
{
    public class CourseService : ICourseService
    {
        ...
        {
            ICollection<UserCourse> courses = await _context.UserCourses.Include(c => c.Course)
        ...
        public async Task<ICollection<CourseModel>> GetAvailableCoursesAsync(int studentId)
        {
            var courses = await _context.Courses.Include(c => c.UserCourses)
                                                 .Where(uc => uc.UserCourses
                                                  .All(u => u.StudentId != studentId) || uc.UserCourses.Count == 0)
                                                   .ToListAsync();
